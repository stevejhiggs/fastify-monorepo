 ...
 * using the OpenTelemetry API, wrapped in a traced span. Run with
 * `pnpm dev:with-telemetry` to see metrics and traces exported to the console.
 */
import { metrics, withSpan } from '@repo/open-telemetry';
...
export default function registerRoutes(app: EnhancedFastifyInstance) {
  app.get('/metrics-example', async (req, reply) => {
    return withSpan('metricsExample', async (span) => {
      ...
      span.setAttribute('processing.duration_ms', Math.round(duration));
