...
Fastify plugin for request-scoped logging. Provides a persistent logger instance that's available throughout the entire request lifecycle, accessible from anywhere in your application.
...
```bash
pnpm add @repo/fastify-logging fastify @repo/logging
```
...
```typescript
import { logger } from '@repo/fastify-logging';
